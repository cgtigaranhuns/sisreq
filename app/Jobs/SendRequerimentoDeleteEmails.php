...
use App\Mail\RequerimentoExcluido;
...
class SendRequerimentoDeleteEmails implements ShouldQueue
{
        ...
        try {
            ...
            if ($adminEmail) {
                Mail::to($adminEmail)->send(
                    new RequerimentoExcluido($this->requerimento, $discente, 'admin')
                );
            }
            ...
            if ($discente->email) {
                Mail::to($discente->email)->send(
                    new RequerimentoExcluido($this->requerimento, $discente, 'discente')
                );
            }

        } catch (\Exception $e) {
            Log::error("Erro ao enviar e-mails de exclusão para o requerimento ID: {$this->requerimento->id}. Erro: " . $e->getMessage());
        }
    }

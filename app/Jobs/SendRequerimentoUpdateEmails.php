...
class SendRequerimentoUpdateEmails implements ShouldQueue
{
        ...
        try {
            ...
            if ($adminEmail) {
                Mail::to($adminEmail)->send(
                    new RequerimentoAtualizado($this->requerimento, $discente, 'admin')
                );
            }
            ...
            if ($discente->email) {
                Mail::to($discente->email)->send(
                    new RequerimentoAtualizado($this->requerimento, $discente, 'discente')
                );
            }

        } catch (\Exception $e) {
            Log::error("Erro ao enviar e-mails de atualização para o requerimento ID: {$this->requerimento->id}. Erro: " . $e->getMessage());
        }
    }

...
class MultiLdapUserProvider implements UserProvider
{
    ...
    public function retrieveByCredentials(array $credentials)
    {
        ...
        // Primeiro tenta encontrar o usuário local
        $user = User::where('matricula', $credentials['matricula'])->first();
        
        if ($user) {
            \Log::debug("Usuário local encontrado", ['matricula' => $credentials['matricula']]);
            return $user;
        }
        
        // Se não encontrou localmente, busca no LDAP
        $connection = $credentials['connection'] ?? 'adm';
        \Log::debug("Tentando autenticar na conexão LDAP: " . $connection);
        
        $ldapUser = $this->findLdapUser($credentials['matricula'], $connection);
        
        if (!$ldapUser) {
            \Log::debug("Usuário não encontrado na conexão LDAP: " . $connection);
            return null;
        }
    ...
    public function validateCredentials(Authenticatable $user, array $credentials)
    {
        ...
        // Primeiro tenta autenticar com a senha local
        if (Hash::check($credentials['password'], $user->password)) {
            \Log::debug("Autenticação local bem-sucedida", ['matricula' => $credentials['matricula']]);
            return true;
        }
        
        \Log::debug("Autenticação local falhou, tentando LDAP", ['matricula' => $credentials['matricula']]);
        
        // Se falhou, tenta autenticar no LDAP
        $connection = $credentials['connection'] ?? 'adm';
        $ldapResult = $this->authenticateInLdap($user, $credentials, $connection);
        
        // Se autenticou no LDAP, atualiza a senha local
        if ($ldapResult) {
            \Log::debug("Autenticação LDAP bem-sucedida, atualizando senha local", ['matricula' => $credentials['matricula']]);
            $user->password = bcrypt($credentials['password']);
            $user->save();
            return true;
        }
        
        \Log::warning("Falha na autenticação local e LDAP", ['matricula' => $credentials['matricula']]);
        return false;
    }
    ...
    protected function authenticateInLdap($user, $credentials, $connection)
    {
        ...
        try {
            ...
            \Log::error("Erro durante autenticação LDAP", [
                'error' => $e->getMessage(),
                'matricula' => $credentials['matricula']
            ]);
            return false;
    ...
    protected function getOrCreateLocalUser($ldapUser, $credentials)
    {
        ...
        $newUser->password = bcrypt($credentials['password']); // Armazena a senha LDAP localmente
